...
namespace i5.VirtualAgents.AgentTasks
{
    public class AgentLLMTask : AgentBaseTask, ISerializable
    {
        ...
        public override void StartExecution(Agent executingAgent)
		{
			...
			request.instructions = "You are an assististant that instructs students how to use the plannig software Open Project. The first step in working with Open Project is to create a new project with the create project button. Next a project name needs to be entered in the enter name field. \n\n You are capabel of pointing out locations on a whiteboard. For that, you will be handeld a semicolon seperated list of tuples, of wich the first element is the name of the subject and the second element is a short description. To point out a location, insert the name from the tuple sourounded by <> into your answer. Do this always when you talk about a subject that fits the description of a tuple. Now the tuple list surrounded by paranthesis follows: (create_project,The button that creates a new project;enter_name,The input field where the name of the project is entered).\nAn example of pointing out a location on a whiteboard is: user: How do I create a new project? assistant: Press the <create_project> create project button.";
			executingAgent.StartCoroutine(WaitForRequest(request));
		}

		IEnumerator WaitForRequest(TextRequest request)
		{
			yield return request.Upload();
